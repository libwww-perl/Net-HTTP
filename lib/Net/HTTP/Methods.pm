...
sub http_configure {
    ...
    $cnf->{"PeerPort"}
        = $peer_uri->_port || $cnf->{PeerPort} || $self->http_default_port;
    ...
    if ( ($host) || ( !$explicit_host ) ) {
        my $uri
            = ($explicit_host) ? URI->new("http://$host") : $peer_uri->clone;
        if ( !$uri->_port ) {
        ...
        my $remove
            = ":"
            . $self
            ->http_default_port;   # we want to remove the default port number
...
for my $method (
    qw(host keep_alive send_te max_line_length max_header_lines peer_http_version)
) {
...
sub format_request {
    ...
    return _bytes(
        join( $CRLF, "$method $uri HTTP/$ver", @h2, @h, "", $content ) );
}
...
sub write_chunk {
    ...
    $self->print(
        _bytes( sprintf( "%x", length( $_[0] ) ) . $CRLF . $_[0] . $CRLF ) );
}
...
sub can_read {
    ...
    return 1
        if $self->isa('Net::SSL') && $self->can('pending') && $self->pending;
...
sub read_entity_body {
    ...
    if ( ${*$self}{'http_first_body'} ) {
        ...
        if ( $method eq "HEAD" ) {
            ...
            pop(@te)
                while @te
                && $te[-1] eq "chunked";    # ignore repeated chunked spec
                ...
                if ( $_ eq "deflate" && inflate_ok() ) {
                    ...
                    $_ = sub {
                        ...
                        IO::Uncompress::Gunzip::gunzip(
                            ...
                            or die
                            "Can't gunzip content: $IO::Uncompress::Gunzip::GunzipError";
                        return \$output;
                    };
                }
                elsif ( $_ eq "identity" ) {
            ...
            defined( my $content_length = ${*$self}{'http_content_length'} ) )
        {
